...
select subscription_status, count(customer_id), sum(purchase_amount) as total_revenue , avg(purchase_amount) as average_spend from customer group by subscription_status order by total_revenue and average_spend;
...
select age_group, sum(purchase_amount) as total_revenue from customer group by age_group order by total_revenue;